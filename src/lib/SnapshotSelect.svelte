...
<div class="snapshot-select" bind:this={root}>
  ...
  <button class="snapshot-select-trigger" aria-expanded={expanded} onclick={() => expanded = !expanded}>
  ...
  {#if expanded}
    <div class="snapshot-select-options" role="group" aria-label={label}>
      {#each captures as capture}
        <button class:active={capture.sequence === value} aria-pressed={capture.sequence === value} onclick={() => { onchange(capture.sequence); expanded = false; root.querySelector('button')?.focus(); }}>
